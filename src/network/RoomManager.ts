...
export class RoomManager {
   ...
   */
  async pushCommand(command: object): Promise<void> {
    if (!this.roomId) throw new Error('No room');
    const db = getDb();
   ...
   */
  onCommand(callback: (cmd: Record<string, unknown>) => void): () => void {
    if (!this.roomId) throw new Error('No room');
    const db = getDb();
    const unsub = onChildAdded(ref(db, `rooms/${this.roomId}/commands`), (snap) => {
      if (snap.exists()) callback(snap.val() as Record<string, unknown>);
    });
    this.unsubs.push(unsub);
    return unsub;
  }
