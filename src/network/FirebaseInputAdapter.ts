...
export class FirebaseInputAdapter implements InputAdapter {
  ...
  constructor(
    ...
    this.unsubCommand = this.roomManager.onCommand((raw) => {
      const cmd = raw as unknown as SerializableCommand & { timestamp?: number };
      const key = `${cmd.type}-${cmd.payload?.playerId}-${cmd.payload?.expression}-${cmd.payload?.direction}`;
  ...
  submitMove(expression: string, direction: 1 | -1): void {
    ...
    // Mark as processed so we don't double-execute from Firebase
    const key = `${serialized.type}-${serialized.payload.playerId}-${serialized.payload.expression}-${serialized.payload.direction}`;
    this.processedCommands.add(key);

    cmd.execute();
    this.roomManager.pushCommand(serialized);
